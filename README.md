...
- **波形モニタ**: 速度・位置・トルクを **1 枚のグラフ**にまとめて表示。チェックボックスで
  ノード × 系列ごとに表示を切り替えられる。グラフ内に**凡例**（色・名前・現在値）、
  **X 軸の時刻ラベル**（シミュレーション時刻 [s]）、**Y 軸の目盛り**を描く。
  縦軸は**単位ごとの共通スケール**（速度どうしは同じ物差しで比較でき、単位の違う位置・
  トルクは別スケール）。Y 軸は左右 2 本までなので、3 つ目の単位を出したときは
  凡例に「軸なし」と書く
