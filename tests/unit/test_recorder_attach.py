...
def test_listener_is_callable_like_python_can_notifier_calls_it():
    # python-can の Notifier は listener.on_message_received(msg) ではなく
    # listener(msg) という呼び出し可能形式でリスナーを呼ぶ。can.Listener を
    # 継承していないとここで TypeError: 'FrameListener' object is not
    # callable になり、受信スレッドごと止まる（実機で発見されたバグ）。
    network = FakeNetwork()
    recorder = Recorder(None)
    attach_recorder(network, recorder, SimClock(realtime=False))
    listener = network.listeners[0]
    listener(FakeMessage(0x601, bytes([0x40, 0x41, 0x60, 0x00, 0, 0, 0, 0])))
    frames = recorder.recent_frames()
    assert len(frames) == 1
    assert frames[0]["can_id"] == 0x601
    recorder.close()
